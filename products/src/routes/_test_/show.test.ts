...
it('return a 404 if the product is not found', async () => {
  const response = await request(app).get('/api/products/nonexistentid').send().expect(404)
})
...
it('returns the product if found', async () => {
  ...
  // Lấy product vừa tạo
  const getResponse = await request(app).get(`/api/products/${productId}`).send().expect(200)
