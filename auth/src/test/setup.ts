...
global.signup = async (email = 'testemail@example.com', password = 'password123') => {
  const res = await request(app).post('/api/users/signup').send({ email, password })
...
global.signin = async (email = 'testemail@example.com', password = 'password123') => {
  // Try sign in first
  let res = await request(app).post('/api/users/signin').send({ email, password })

  if (res.status !== 200) {
    // If user doesn't exist, create then sign in
    await global.signup(email, password)
    res = await request(app).post('/api/users/signin').send({ email, password })
  }
