...
const CartPage = ({ currentUser }) => {
    ...
    const { doRequest, errors } = UseRequest({
        ...
        body: {},
        onSuccess: (data) => {
            router.push(data?.id ? `/orders/${data.id}` : '/orders')
        }
    })
    ...
    // Guard against undefined items/products (e.g. race conditions after removal)
    const safeItems = (cart?.items || []).filter(i => i && i.product && typeof i.product.price === 'number' && typeof i.quantity === 'number')
    const totalPrice = safeItems.reduce((sum, item) => sum + (item.product.price * item.quantity), 0)
            ...
            {!cart || cart.items?.length === 0 ? (
                <div className="card card-product">
                    <div className="card-body text-center py-5">
                        <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" fill="currentColor" className="text-muted mb-3" viewBox="0 0 16 16">
                        ...
                        <div className="card card-product">
                            <div className="card-body">
                                ...
                                {safeItems.map((item) => {
                                    ...
                                    const isOutOfStock = typeof item.product.quantity === 'number' && item.product.quantity < item.quantity
                                    
                                    return (
                                        <div key={item.product.id} className="d-flex align-items-center justify-content-between border-bottom pb-3 mb-3">
                                            <div className="flex-grow-1">
                                                <Link href={`/products/${item.product.id}`} className="text-decoration-none">
                        ...
                        <div className="card card-product">
                            <div className="card-body">
                                ...
                                <button 
                                    ...
                                    onClick={() => {
                                        // Build items payload from current cart state
...
export const getServerSideProps = withCurrentUser(async (context) => {
    // Just need currentUser from withCurrentUser
    // Cart data will be fetched client-side via context
    return {}
})
