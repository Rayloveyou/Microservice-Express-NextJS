...
const OrderIndex = ({ orders, currentUser }) => {
    const orderList = orders.map((order) => {
        ...
        const firstTitle = order.items && order.items.length > 0
            ? (order.items[0].titleSnapshot || order.items[0].product?.title || 'N/A')
            : 'N/A'
        const itemCount = order.items ? order.items.length : 0
        const totalUnits = order.items ? order.items.reduce((s,i)=>s + (i.quantity||0),0) : (order.quantity || 0)
        ...
        return (
            <tr key={order.id}>
                <td>
                    ...
                    {itemCount > 1 && (
                        <div className="small text-muted">+ {itemCount - 1} more item(s)</div>
                    )}
                </td>
                <td>{totalUnits}</td>
                ...
                <td>
                    {order.status === 'Created' || order.status === 'AwaitingPayment' ? (
